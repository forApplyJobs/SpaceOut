...
public:
  ...
  Sprite(Bitmap* pBitmap, RECT& rcBounds, int iHealth,
      BOUNDSACTION baBoundsAction = BA_STOP);
  ...
  void SetBitmap(Bitmap* _bitmap) { m_pBitmap = _bitmap; };
  ...
  void    SetPrevPosition(RECT& rcPrev) { m_rcPrev = rcPrev; };
  RECT&    GetPrevPosition() {return m_rcPrev; };
  ...
  void    SetHealth(int newHealth) { m_iHealth = newHealth; };
  void    Setbfrm(int bfrm) { m_bfrm = bfrm; };
  int    Getbfrm() { return m_bfrm; };
